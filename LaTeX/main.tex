...
Per ogni parallelogramma $P$, numero positivo $\varepsilon$ e intervalli di direzioni $I, J$ tale che $\operatorname{cl}(I) \subseteq \operatorname{int}(J)$, esistono dei sotto parallelogrammi $P_{1}, P_{2}, \ldots$ tali che:
\begin{itemize}
...
Partendo dai parallelogrammi $P_1, P_2,...$, vogliamo costruire induttivamente una collezione numerabile di parallelogrammi indicizzata dalle sequenze finite di naturali.\\
Sia $\mathbf{n}=n_{1} n_{2} \cdots n_{k}$ una sequenza finita di naturali. Supponiamo che siano già stati definiti $P=P_{\mathbf{n}}$ e $I=I_{\mathbf{n}}$, allora definiamo $P_{\mathbf{n} j}, I_{\mathbf{n} j}$ per ogni $j \in \mathbb{N}$ positivo come segue.\\

Scegliamo dei sotto-parallelogrammi aperti $P^{1}, P^{2}, \ldots$ di $P=P_{\mathbf{n}}$ e dei sotto-intervalli aperti $I^{1}, I^{2}, \ldots$ di $I=I_{\mathrm{n}}$ con $\operatorname{cl}(I^j) \subseteq \operatorname{int}(I)$ tali che per ogni $\ell \in A_{P, I}$, l'insieme di sotto-parallelogrammi $\left\{P^{j}: \ell \in A_{P^{j}, I^{j}}\right\}$
sia un ricoprimento di Vitali del segmento $\ell \cap P$. \\
Per ogni $j$ naturale positivo, scegliamo  $\varepsilon^{j} >0$ in modo che $\sum_j \e^j < \e_{\mathbf{n}}$ e un intervallo di direzioni $J^{j}$ tale che $\operatorname{cl}\left(I^{j}\right) \subseteq \operatorname{int}\left(J^{j}\right)$ e $\left|J^{j} \backslash I^{j}\right|<\varepsilon_\mathbf{n}$, dove intendiamo l'usuale misura di lunghezza sull'insieme delle direzioni.\\ 
...
Ricordiamo che l'insieme $\left\{P^{j}: \ell \in A_{P^{j}, I^{j}}\right\}$ è un ricoprimento di Vitali di $\ell \cap P$ per ogni $\ell \in A_{P^, I}$. In particolare, dato un punto $x \in \ell \cap P$, allora $x \in P^j$ per un certo $j$ e la direzione di $\ell$ appartiene a $I_j$.\\
...
Ricordando che il ricoprimento è di Vitali, preso $x \in \ell \cap P$, si può anche richiedere che $P^j$ come sopra abbia diametro piccolo a piacere.\\
...
copre un sottoinsieme aperto denso del segmento $\ell \cap P$. In particolare, $\bigcup_m P_{\mathbf{n}m} \cap \ell$ denso in $P_\mathbf{n} \cap \ell$.\\
...
Se $\ell \notin L$, si hanno due possibilità: o $\ell$ non interseca $P_{\mathbf{n}}$ per ogni sequenza $\mathbf{n}$, oppure esiste $\mathbf{n}$ per cui la direzione di $\ell$ non appartiene a $I_{\mathbf{n}}$.\\
Nel primo caso, $\ell$ non interseca $A$.\\
Nel secondo caso, posso supporre che la direzione $d$ di $\ell$ non appartenga neanche a $\operatorname{cl}(I_\mathbf{n})$: altrimenti basta prendere un parallelogramma $P_{\mathbf{n}m} \subseteq P_{\mathbf{n}}$ che interseca $\ell$ e, siccome $\operatorname{cl}(I_{\mathbf{n}m}) \subseteq \operatorname{int}(I_{\mathbf{n}})$, $d \not \in \operatorname{cl}(I_{\mathbf{n}m})$.\\
